...
export function ProfileModal({ 
  ...
  const handleConfirmDeleteAccount = async () => {
    if (isGuest) {
      setDeleteError('Guest users cannot delete a guest account.');
      return;
    }
  ...
  const deletionCard = isAdmin
    ? 'border-slate-800 bg-[#0d1321]/70'
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className={`p-5 rounded-xl ${cardBg} border ${borderCol} flex flex-col items-center text-center relative`}>
            ...
            {isAdmin && !currentUser.isAnonymous && (
              <span className="absolute top-3 right-3 px-2 py-0.5 text-[9px] font-bold tracking-wider uppercase font-mono bg-blue-500/15 text-blue-400 border border-blue-500/20 rounded-full flex items-center gap-1">
            ...
            {isAdmin && onOpenAdminPortal && (
              <button
                ...
                className="mt-4 w-full bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold py-2.5 px-4 rounded-lg transition flex items-center justify-center gap-2"
              >
          ...
          {/* Account deletion */}
          <div className={`border ${deletionCard} rounded-xl p-3 space-y-1.5`}>
            <h4 className={`text-[11px] font-bold ${isAdmin ? 'text-slate-400' : 'text-slate-500'} flex items-center gap-2`}>
              <AlertTriangle className="w-3.5 h-3.5 text-red-400" />
              Account deletion
            </h4>
            <p className={`text-[10px] ${textSecondary} leading-relaxed`}>
              Permanently delete this account and its account data. This cannot be undone.
            </p>

            {isGuest ? (
              <p className="text-xs text-amber-500 font-medium">
                Guest users cannot delete a guest account.
              </p>
            ) : showDeleteConfirm ? (
              <div className="space-y-3 bg-red-500/5 border border-red-500/20 rounded-lg p-3 animate-in slide-in-from-top-2 duration-150">
                <p className="text-xs text-red-400 font-semibold">
                  Are you absolutely sure? All of your account data will be permanently deleted.
                </p>
                <div>
              ...
              <button
                ...
                className="w-full sm:w-auto bg-transparent hover:bg-red-500/10 text-red-500 border border-red-500/20 hover:border-red-500/35 text-[11px] font-semibold py-1.5 px-3 rounded-lg transition flex items-center justify-center gap-1.5"
              >
                <Trash2 className="w-3.5 h-3.5" />
                Delete account
              </button>
